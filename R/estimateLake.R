...
#'   minimum and maximum in dB, default c(-60, -26).
#' @param TSthresh
...
#'   generated, default c(106, 109, 129, 130, 202, 204, 504).
#' @param region
...
estimateLake <-
  function (maindir,
            ...
            soi = c(106, 109, 129, 130, 202, 204, 504),
            spInfo,
            sliceDef,
            short = TRUE,
            IntMeansPlots = FALSE,
            descr = "ACMT Estimates")
  {
    ...
    normalized_path <- gsub("\\\\", "/", gsub("\\", "/", sv$EV_filename, fixed = TRUE))
    EV_filename_parts <- strsplit(normalized_path, "[/]")
    sv$EVfolder <- sapply(EV_filename_parts, function(x) if(length(x) >= 7) x[7] else NA)
    ...
    para(
      "Created using the R package EchoNet2Fish (https://github.com/phillipskr/EchoNet2Fish), written by Jean V. Adams, Dave Warner, and Kristy Phillips."
    )
